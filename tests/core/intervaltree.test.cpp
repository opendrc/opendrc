...
TEST_SUITE("[OpenDRC] odrc::core interval tree tests") {
  TEST_CASE("test overlapping cells") {
    ...
    std::vector<std::vector<int>> mbr{
        ...
        {14, 16, 8, 18}, {1, 3, 9, 19}, {5, 13, 0, 12}};//the data represents {x_min,x_max,y_min,y_max}
    for (int i = 0; i < 9; i++) {
      sorted_edge.emplace_back(odrc::core::interval{
          mbr[i][0], mbr[i][1], mbr[i][2], sorted_edge.size() / 2 + 1});//input {x_min, x_max, y_min,id}
      sorted_edge.emplace_back(odrc::core::interval{
          mbr[i][0], mbr[i][1], mbr[i][3], -sorted_edge.size() / 2 - 1});//input {x_min, x_max, y_max,-id}
    }
    ...
    auto brute_force_query_result = brute_force_query(mbr);// get brute force query result
    CHECK_EQ(overlap_cells.size(), brute_force_query_result.size());//check the size of result
    for (int i = 0; i != brute_force_query_result.size(); i++) {
      CHECK_EQ(overlap_cells.at(i), brute_force_query_result.at(i));//check every pair
    }
  }
}